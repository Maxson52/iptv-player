...
import type { ChannelEpg, EpgProgram } from "../../+page.server";
...
export const GET: RequestHandler = async () => {
  try {
    const parsedEpg = await fetchAndParseEpgXml();
    if (!parsedEpg?.programmes) {
      return json({});
    }
    const epg = resolveEpg(parsedEpg.programmes);
    return json(epg);
  } catch (err) {
